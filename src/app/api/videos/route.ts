...
// Exporting a function named after an HTTP method is how a route handler declares what it supports.
// The name `GET` is required and case-sensitive — rename it and the endpoint stops responding, returning
// 405 Method Not Allowed instead. It is marked `async` so it *could* `await` things; the file operations
// used below are the synchronous variants, so there is no `await` in practice, but `async` is harmless
// and standard here because route handlers are expected to return a promise.
export async function GET() {
  ...
  try {
    ...
    const videos = files
      // Keep only the entries that look like video files.
      .filter(file => {
